...
@pytest.mark.asyncio
async def test_ensure_success(cosmos_client, mock_cosmos_clients):
...
@pytest.mark.asyncio
async def test_ensure_fail_when_client_is_none():
...
@pytest.mark.asyncio
async def test_ensure_database_read_fails(cosmos_client, mock_cosmos_clients):
...
@pytest.mark.asyncio
async def test_ensure_container_read_fails(cosmos_client, mock_cosmos_clients):
    _, db, container = mock_cosmos_clients
    db.read = AsyncMock(return_value=True)
...
def test_constructor_invalid_credential():
...
def test_constructor_invalid_database():
...
def test_constructor_invalid_container():
    cosmos_mock = MagicMock()
    db_mock = MagicMock()
...
@pytest.mark.asyncio
async def test_create_conversation_success(cosmos_client, mock_cosmos_clients):
...
@pytest.mark.asyncio
async def test_create_conversation_failure(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
...
@pytest.mark.asyncio
async def test_upsert_conversation_success(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
...
@pytest.mark.asyncio
async def test_upsert_conversation_failure(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
    container.upsert_item = AsyncMock(return_value=None)
    result = await cosmos_client.upsert_conversation({"id": "x"})
    assert result is False
...
@pytest.mark.asyncio
async def test_get_conversation_found(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
...
@pytest.mark.asyncio
async def test_get_conversation_not_found(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
...
@pytest.mark.asyncio
async def test_get_conversations_with_limit(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
...
@pytest.mark.asyncio
async def test_create_message_with_feedback(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
...
@pytest.mark.asyncio
async def test_create_message_without_feedback(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
...
@pytest.mark.asyncio
async def test_create_message_conversation_not_found(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
    cosmos_client.enable_message_feedback = True
...
@pytest.mark.asyncio
async def test_update_message_feedback_success(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
...
@pytest.mark.asyncio
async def test_update_message_feedback_not_found(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
...
@pytest.mark.asyncio
async def test_get_messages(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
    container.query_items.return_value = AsyncIteratorWrapper([
        {"id": "m1"}, {"id": "m2"}
    ])
...
@pytest.mark.asyncio
async def test_delete_messages_with_messages(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
...
@pytest.mark.asyncio
async def test_delete_conversation_found(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
...
@pytest.mark.asyncio
async def test_delete_conversation_not_found(cosmos_client, mock_cosmos_clients):
    _, _, container = mock_cosmos_clients
    container.read_item = AsyncMock(return_value=None)
